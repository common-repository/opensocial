...
class OpenSocial_Saml2_Response
{
     ...
     */
    public function __construct(OpenSocial_Saml2_Settings $settings, $response)
    {
        ...
        $this->document = OpenSocial_Saml2_Utils::loadXML($this->document, $this->response);
        if (!$this->document) {
        ...
        try {
            ...
            if ($this->document->documentElement->getAttribute('Version') != '2.0') {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::UNSUPPORTED_SAML_VERSION
                );
            }
            ...
            if (!$this->document->documentElement->hasAttribute('ID')) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::MISSING_ID
                );
            }
            ...
            if (!$singleAssertion) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::WRONG_NUMBER_OF_ASSERTIONS
                );
            }
            ...
            $responseTag = '{'.OpenSocial_Saml2_Constants::NS_SAMLP.'}Response';
            $assertionTag = '{'.OpenSocial_Saml2_Constants::NS_SAML.'}Assertion';
            ...
            if ($this->_settings->isStrict()) {
                ...
                if ($security['wantXMLValidation']) {
                    ...
                    if ($this->encrypted) {
                        $res = OpenSocial_Saml2_Utils::validateXML($this->decryptedDocument, 'saml-schema-protocol-2.0.xsd', $this->_settings->isDebugActive());
                        if (!$res instanceof DOMDocument) {
                            throw new OpenSocial_Saml2_ValidationError(
                                $errorXmlMsg,
                                OpenSocial_Saml2_ValidationError::INVALID_XML_FORMAT
                ...
                if (!$this->encrypted && $security['wantAssertionsEncrypted']) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::NO_ENCRYPTED_ASSERTION
                    );
                }
                ...
                if ($security['wantNameIdEncrypted']) {
                    ...
                    if ($encryptedIdNodes->length != 1) {
                        throw new OpenSocial_Saml2_ValidationError(
                            ...
                            OpenSocial_Saml2_ValidationError::NO_ENCRYPTED_NAMEID
                        );
                    }
                }
                ...
                if (!$this->checkOneCondition()) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::MISSING_CONDITIONS
                    );
                }
                ...
                if (!$this->checkOneAuthnStatement()) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::WRONG_NUMBER_OF_AUTHSTATEMENTS
                    );
                }
                ...
                if ($encryptedAttributeNodes->length > 0) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::ENCRYPTED_ATTRIBUTES
                    );
                }
                    ...
                    if (empty($destination)) {
                        if (!$security['relaxDestinationValidation']) {
                            throw new OpenSocial_Saml2_ValidationError(
                                ...
                                OpenSocial_Saml2_ValidationError::EMPTY_DESTINATION
                            );
                        }
                    } else {
                        if (strpos($destination, $currentURL) !== 0) {
                            $currentURLNoRouted = OpenSocial_Saml2_Utils::getSelfURLNoQuery();
                ...
                if (!empty($validAudiences) && !in_array($spEntityId, $validAudiences, true)) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::WRONG_AUDIENCE
                    );
                }
                ...
                if (!empty($sessionExpiration) && $sessionExpiration + OpenSocial_Saml2_Constants::ALLOWED_CLOCK_DRIFT <= time()) {
                    throw new OpenSocial_Saml2_ValidationError(
                        "The attributes have expired, based on the SessionNotOnOrAfter of the AttributeStatement of this Response",
                        OpenSocial_Saml2_ValidationError::SESSION_EXPIRED
                    );
                }
                ...
                foreach ($subjectConfirmationNodes as $scn) {
                    if ($scn->hasAttribute('Method') && $scn->getAttribute('Method') != OpenSocial_Saml2_Constants::CM_BEARER) {
                        continue;
                    }
                    ...
                    if ($subjectConfirmationDataNodes->length == 0) {
                        ...
                        if ($scnData->hasAttribute('NotOnOrAfter')) {
                            $noa = OpenSocial_Saml2_Utils::parseSAML2Time($scnData->getAttribute('NotOnOrAfter'));
                            if ($noa + OpenSocial_Saml2_Constants::ALLOWED_CLOCK_DRIFT <= time()) {
                                continue;
                            }
                        }
                        if ($scnData->hasAttribute('NotBefore')) {
                            $nb = OpenSocial_Saml2_Utils::parseSAML2Time($scnData->getAttribute('NotBefore'));
                            if ($nb > time() + OpenSocial_Saml2_Constants::ALLOWED_CLOCK_DRIFT) {
                                continue;
                            }
                        }
                ...
                if (!$anySubjectConfirmation) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::WRONG_SUBJECTCONFIRMATION
                    );
                }
                ...
                if ($security['wantAssertionsSigned'] && !$hasSignedAssertion) {
                    throw new OpenSocial_Saml2_ValidationError(
                        ...
                        OpenSocial_Saml2_ValidationError::NO_SIGNED_ASSERTION
                    );
                }
            ...
            if ($this->encrypted) {
                $encryptedIDNodes = OpenSocial_Saml2_Utils::query($this->decryptedDocument, '/samlp:Response/saml:Assertion/saml:Subject/saml:EncryptedID');
                if ($encryptedIDNodes->length > 0) {
                    throw new OpenSocial_Saml2_ValidationError(
                        'Unsigned SAML Response that contains a signed and encrypted Assertion with encrypted nameId is not supported.',
                        OpenSocial_Saml2_ValidationError::NOT_SUPPORTED
                    );
                }
            }
            ...
            if (empty($signedElements) || (!$hasSignedResponse && !$hasSignedAssertion)) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::NO_SIGNATURE_FOUND
                );
            } else {
                ...
                if ($hasSignedResponse && !OpenSocial_Saml2_Utils::validateSign($this->document, $cert, $fingerprint, $fingerprintalg, OpenSocial_Saml2_Utils::RESPONSE_SIGNATURE_XPATH, $multiCerts)) {
                    throw new OpenSocial_Saml2_ValidationError(
                ...
                if ($hasSignedAssertion && !OpenSocial_Saml2_Utils::validateSign($documentToCheckAssertion, $cert, $fingerprint, $fingerprintalg, OpenSocial_Saml2_Utils::ASSERTION_SIGNATURE_XPATH, $multiCerts)) {
                    throw new OpenSocial_Saml2_ValidationError(
                        "Signature validation failed. SAML Response rejected",
                        OpenSocial_Saml2_ValidationError::INVALID_SIGNATURE
                    );
     ...
     *
     * @throws OpenSocial_Saml2_ValidationError If status is not success
     */
    public function checkStatus()
    {
        $status = OpenSocial_Saml2_Utils::getStatus($this->document);

        if (isset($status['code']) && $status['code'] !== OpenSocial_Saml2_Constants::STATUS_SUCCESS) {
            ...
            throw new OpenSocial_Saml2_ValidationError(
                $statusExceptionMsg,
                OpenSocial_Saml2_ValidationError::STATUS_CODE_IS_NOT_SUCCESS
            );
        }
    }
    ...
    */
    ...
    public function getIssuers()
    {
        ...
        $responseIssuer = OpenSocial_Saml2_Utils::query($this->document, '/samlp:Response/saml:Issuer');
        if ($responseIssuer->length > 0) {
            if ($responseIssuer->length == 1) {
                ...
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::ISSUER_MULTIPLE_IN_RESPONSE
                );
            }
        }
        ...
        if ($assertionIssuer->length == 1) {
            ...
            throw new OpenSocial_Saml2_ValidationError(
                ...
                OpenSocial_Saml2_ValidationError::ISSUER_NOT_FOUND_IN_ASSERTION
            );
        }
    ...
    public function getNameIdData()
    {
        ...
        if ($encryptedIdDataEntries->length == 1) {
            ...
            $nameId = OpenSocial_Saml2_Utils::decryptElement($encryptedData, $seckey);
        ...
        if (!isset($nameId)) {
            ...
            if ($security['wantNameId']) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::NO_NAMEID
                );
            }
        } else {
            if ($this->_settings->isStrict() && empty($nameId->nodeValue)) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::EMPTY_NAMEID
                );
            }
                    ...
                    if ($this->_settings->isStrict() && $attr == 'SPNameQualifier') {
                        ...
                        if ($spEntityId != $nameId->getAttribute($attr)) {
                            throw new OpenSocial_Saml2_ValidationError(
                                ...
                                OpenSocial_Saml2_ValidationError::SP_NAME_QUALIFIER_NAME_MISMATCH
                            );
                        }
                    }
    ...
    public function getSessionNotOnOrAfter()
    {
        ...
        if ($entries->length !== 0) {
            $notOnOrAfter = OpenSocial_Saml2_Utils::parseSAML2Time($entries->item(0)->getAttribute('SessionNotOnOrAfter'));
        }
        return $notOnOrAfter;
    }
        ...
        foreach ($entries as $entry) {
            ...
            if (in_array($attributeKeyName, array_keys($attributes))) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::DUPLICATED_ATTRIBUTE_NAME_FOUND
                );
            }
    ...
    public function processSignedElements()
    {
        ...
        foreach ($signNodes as $signNode) {
            ...
            if ($signedElement != $responseTag && $signedElement != $assertionTag) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::WRONG_SIGNED_ELEMENT
                );
            }
            ...
            if (empty($idValue)) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::ID_NOT_FOUND_IN_SIGNED_ELEMENT
                );
            }
            ...
            if (in_array($idValue, $verifiedIds)) {
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::DUPLICATED_ID_IN_SIGNED_ELEMENTS
                );
            }
            $verifiedIds[] = $idValue;
            ...
            if ($ref->length == 1) {
                ...
                if (!empty($sei)) {
                    ...
                    if ($sei != $idValue) {
                        throw new OpenSocial_Saml2_ValidationError(
                            ...
                            OpenSocial_Saml2_ValidationError::INVALID_SIGNED_ELEMENT
                        );
                    }
                    ...
                    if (in_array($sei, $verifiedSeis)) {
                        throw new OpenSocial_Saml2_ValidationError(
                            ...
                            OpenSocial_Saml2_ValidationError::DUPLICATED_REFERENCE_IN_SIGNED_ELEMENTS
                        );
                    }
                    $verifiedSeis[] = $sei;
                }
                ...
                throw new OpenSocial_Saml2_ValidationError(
                    ...
                    OpenSocial_Saml2_ValidationError::UNEXPECTED_REFERENCE
                );
            }
        ...
        if (!empty($signedElements) && !$this->validateSignedElements($signedElements)) {
            throw new OpenSocial_Saml2_ValidationError(
                ...
                OpenSocial_Saml2_ValidationError::UNEXPECTED_SIGNED_ELEMENTS
            );
        }
    ...
    public function validateTimestamps()
    {
        ...
        for ($i = 0; $i < $timestampNodes->length; $i++) {
            ...
            if ($nbAttribute && OpenSocial_Saml2_Utils::parseSAML2Time($nbAttribute->textContent) > time() + OpenSocial_Saml2_Constants::ALLOWED_CLOCK_DRIFT) {
                throw new OpenSocial_Saml2_ValidationError(
                    'Could not validate timestamp: not yet valid. Check system clock.',
                    OpenSocial_Saml2_ValidationError::ASSERTION_TOO_EARLY
                );
            }
            if ($naAttribute && OpenSocial_Saml2_Utils::parseSAML2Time($naAttribute->textContent) + OpenSocial_Saml2_Constants::ALLOWED_CLOCK_DRIFT <= time()) {
                throw new OpenSocial_Saml2_ValidationError(
                    'Could not validate timestamp: expired. Check system clock.',
                    OpenSocial_Saml2_ValidationError::ASSERTION_EXPIRED
                );
            }
        }
        return true;
    ...
    public function validateSignedElements($signedElements)
    {
        ...
        // by OpenSocial_Saml2_Utils->validateSign()
        if (in_array($responseTag, $signedElements)) {
            $expectedSignatureNodes = OpenSocial_Saml2_Utils::query($this->document, OpenSocial_Saml2_Utils::RESPONSE_SIGNATURE_XPATH);
            if ($expectedSignatureNodes->length != 1) {
                throw new OpenSocial_Saml2_ValidationError(
                    "Unexpected number of Response signatures found. SAML Response rejected.",
                    OpenSocial_Saml2_ValidationError::WRONG_NUMBER_OF_SIGNATURES_IN_RESPONSE
                );
            }
        }

        if (in_array($assertionTag, $signedElements)) {
            $expectedSignatureNodes = $this->_query(OpenSocial_Saml2_Utils::ASSERTION_SIGNATURE_XPATH);
            if ($expectedSignatureNodes->length != 1) {
                throw new OpenSocial_Saml2_ValidationError(
                    "Unexpected number of Assertion signatures found. SAML Response rejected.",
                    OpenSocial_Saml2_ValidationError::WRONG_NUMBER_OF_SIGNATURES_IN_ASSERTION
                );
            }
        }
    ...
    {
        if ($this->encrypted) {
            return OpenSocial_Saml2_Utils::query($this->decryptedDocument, $query);
        } else {
    ...
    protected function _decryptAssertion($dom)
    {
        ...
        if (empty($pem)) {
            throw new OpenSocial_Saml2_Error(
                ...
                OpenSocial_Saml2_Error::PRIVATE_KEY_NOT_FOUND
            );
        }
        ...
        $check = OpenSocial_Saml2_Utils::loadXML($decrypted, $decryptedXML);
        if ($check === false) {
        ...
        if ($encData->parentNode instanceof DOMDocument) {
            ...
            if (!$decrypted->hasAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:saml')
                ...
                $decrypted->setAttributeNS('http://www.w3.org/2000/xmlns/', $ns, OpenSocial_Saml2_Constants::NS_SAML);
            }

            OpenSocial_Saml2_Utils::treeCopyReplace($encryptedAssertion, $decrypted);

            // Rebuild the DOM will fix issues with namespaces as well
            $dom = new DOMDocument();
            return OpenSocial_Saml2_Utils::loadXML($dom, $container->ownerDocument->saveXML());
        }
    }
