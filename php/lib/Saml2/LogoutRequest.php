 ...
 *
 */
class OpenSocial_Saml2_LogoutRequest
{
    /**
    ...
    */
     ...
     */
    public function __construct(OpenSocial_Saml2_Settings $settings, $request = null, $nameId = null, $sessionIndex = null, $nameIdFormat = null, $nameIdNameQualifier = null)
    {
        ...
        if (!isset($request) || empty($request)) {
            ...
            $nameIdValue = OpenSocial_Saml2_Utils::generateUniqueID();
            ...
            if (!empty($nameId)) {
                if (empty($nameIdFormat) &&
                  $spData['NameIDFormat'] != OpenSocial_Saml2_Constants::NAMEID_UNSPECIFIED) {
                    $nameIdFormat = $spData['NameIDFormat'];
                }
                $spNameQualifier = null;
            } else {
                $nameId = $idpData['entityId'];
                $nameIdFormat = OpenSocial_Saml2_Constants::NAMEID_ENTITY;
                $spNameQualifier = $spData['entityId'];
            }

            $nameIdObj = OpenSocial_Saml2_Utils::generateNameId(
                $nameId,
                $spNameQualifier,
                $nameIdFormat,
                $cert,
...
LOGOUTREQUEST;
    ...
    public static function getID($request)
    {
        if ($request instanceof DOMDocument) {
            ...
            $dom = new DOMDocument();
            $dom = OpenSocial_Saml2_Utils::loadXML($dom, $request);

            if (false === $dom) {
                throw new OpenSocial_Saml2_Error(
                    "LogoutRequest could not be processed",
                    OpenSocial_Saml2_Error::SAML_LOGOUTREQUEST_INVALID
                );
            }
        }
    ...
    public static function getNameIdData($request, $key = null)
    {
        ...
        $encryptedEntries = OpenSocial_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:EncryptedID');
        ...
        if ($encryptedEntries->length == 1) {
            ...
            $entries = OpenSocial_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:NameID');
            if ($entries->length == 1) {
    ...
    public static function getIssuer($request)
    {
        ...
        $issuer = null;
        $issuerNodes = OpenSocial_Saml2_Utils::query($dom, '/samlp:LogoutRequest/saml:Issuer');
    ...
    public static function getSessionIndexes($request)
    {
        ...
        $sessionIndexNodes = OpenSocial_Saml2_Utils::query($dom, '/samlp:LogoutRequest/samlp:SessionIndex');
    ...
    public function isValid($retrieveParametersFromServer = false)
    {
        ...
        try {
            $dom = new DOMDocument();
            $dom = OpenSocial_Saml2_Utils::loadXML($dom, $this->_logoutRequest);
            ...
            if ($this->_settings->isStrict()) {
                ...
                if ($security['wantXMLValidation']) {
                    $res = OpenSocial_Saml2_Utils::validateXML($dom, 'saml-schema-protocol-2.0.xsd', $this->_settings->isDebugActive());
                ...
                if ($dom->documentElement->hasAttribute('NotOnOrAfter')) {
                    $na = OpenSocial_Saml2_Utils::parseSAML2Time($dom->documentElement->getAttribute('NotOnOrAfter'));
                    if ($na <= time()) {
                        throw new OpenSocial_Saml2_ValidationError(
                            "Could not validate timestamp: expired. Check system clock.",
                            OpenSocial_Saml2_ValidationError::RESPONSE_EXPIRED
                        );
                    }
                }
            ...
            if (isset($_GET['Signature'])) {
                $signatureValid = OpenSocial_Saml2_Utils::validateBinarySign("SAMLRequest", $_GET, $idpData, $retrieveParametersFromServer);
                if (!$signatureValid) {
                    throw new OpenSocial_Saml2_ValidationError(
